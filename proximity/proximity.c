...
static void
ble_srv_proximity_update(struct proximity_ctx *ctx, uint16_t val)
{
        srv_char_update(&ctx->proximity, &val);
}

static void
ble_srv_proximity_connected(struct service_desc *s)
{
        ...
        ble_srv_proximity_update(ctx, val);
}

static void
ble_srv_proximity_disconnected(struct service_desc *s)
{
        struct proximity_ctx *ctx = (void *)s;
...
static void
ble_srv_proximity_init(struct proximity_ctx *ctx)
{
        srv_init(ctx, get_vendor_uuid_class(), VENDOR_UUID_SENSOR_SERVICE);
        srv_char_add(ctx, &ctx->proximity,
                     get_vendor_uuid_class(), VENDOR_UUID_PROXIMITY_CHAR,
                     u8"Proximity",
                     2);
        ...
        /*                        0, */
        /*                        0); */
        ctx->connect_cb = ble_srv_proximity_connected;
        ctx->disconnect_cb = ble_srv_proximity_disconnected;
        srv_register(ctx);
}
...
main(void)
{
        ...
        ble_init("proximity");
        ble_srv_tx_init();
        ble_srv_proximity_init(&proximity_ctx);
        ble_srv_ind_init(&ind_ctx);
        ble_adv_start();

        ble_srv_proximity_update(&proximity_ctx, 1234);
